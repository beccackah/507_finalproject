...
# Initialize a SortedDict to store the movie titles sorted by rating
sorted_movies = SortedDict()
...
for title in top_movies.keys():
    ...
    # Add the movie title to the sorted list
    sorted_movies[title] = rating
...
# Print the movie titles sorted by rating using a binary search tree
for title, rating in sorted_movies.items():
    print(title, rating, genres)
